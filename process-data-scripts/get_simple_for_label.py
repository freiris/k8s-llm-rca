...
def extract_simple_results_for_label(input_file):
        ...
        for raw_result in raw_results:
            ...
            for x in analysis:
                ...
                if ('statepath' in x) and (len(x['statepath']) > 0):
                    ...
                    y['statepath'] = unique(y['statepath']) # unique and assign
                ...
                if ('empty_statepath' in x) and (len(x['empty_statepath']) > 0):
                    ...
                    y['empty_statepath'] = unique(y['empty_statepath'])
                ...
                if ('empty_metapath' in x) and (len(x['empty_metapath']) > 0):
                    ...
                    y['empty_metapath'] = unique(y['empty_metapath'])

                simple_result['analysis'].append(y) 
            
            # add label field for human to check
            human_to_label(simple_result)
            print(simple_result)
...
def human_to_label(simple_result):
    extra = {'human_evaluation': {
                 'destkind_relavance': '<High/Moderate/Low/None>',
                 'metapath_is_logical': '<True/False>',
                 'summary_deduces_conclusion': '<True/False>', 
                 'conclusion_explains_error': '<True/False>', 
                 'due_to_discrepancy': '<True/False>',
                 'resolution_is_helpful': '<True/False>',
                 },
             'human_label': '<True/False>',
             'human_conclusion': '<conclusion by human as a reference>'}
    
    simple_result.update(extra)
...
def main():
    input_dir = './output-4-obj-bracket/' # refine it if necessary
    output_dir = './output-4-obj-simple-to-label/'
    ...
    for filename in os.listdir(input_dir):
        ...
        if os.path.isfile(file_path):
            ...
            simple_results = extract_simple_results_for_label(file_path)
