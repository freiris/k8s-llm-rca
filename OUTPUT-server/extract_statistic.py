...
'''
def extract_stat(input_file, output_file):
    ...
    with open(output_file, 'w') as fo:
        for raw_result in raw_results:
            ...
            print(simple_result)
            json_record = json.dumps(simple_result, indent=4)
...
def extract_simple_results(input_file):
    with open(input_file, 'r') as fi:
        ...
        for raw_result in raw_results:
            ...
            for i, j, k in zip(simple_result['cypher_attempts'], simple_result['human_cypher_attempts'], simple_result['is_empty_statepath']):
                if k > 0:
...
def main():
    input_dir = './output-3-server-bracket/'
    output_dir = './output-3-server-statistic/'
